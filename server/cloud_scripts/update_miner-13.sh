...
REPO_URL="https://github.com/RusticLuftig/data-universe.git"
DIR="data-universe"
...
get_version() {
  ...
  version=$(awk -F '"' '/__version__/ {print $2}' neurons/__init__.py | tr -d ' ')
...
if [ -d "$DIR" ]; then
    ...
    if git rev-parse --git-dir > /dev/null 2>&1; then
        ...
        remote_version=$(git show origin/main:neurons/__init__.py | awk -F '"' '/__version__/ {print $2}' | tr -d ' ')
