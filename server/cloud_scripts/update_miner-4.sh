...
REPO_URL="https://github.com/manifold-inc/targon.git"
DIR="targon"
...
get_version() {
  ...
  version=$(awk -F '"' '/__version__/ {print $2}' targon/__init__.py | tr -d ' ')
...
if [ -d "$DIR" ]; then
    ...
    if git rev-parse --git-dir > /dev/null 2>&1; then
        ...
        remote_version=$(git show origin/main:targon/__init__.py | awk -F '"' '/__version__/ {print $2}' | tr -d ' ')
