...
REPO_URL="https://github.com/neuralinternet/compute-subnet.git"
DIR="compute-subnet"
VENV_DIR="venv"
...
install_dependencies() {
    ...
    pip install -r requirements.txt && pip install --no-deps -r requirements-compute.txt && pip install -e .
    deactivate
...
get_version() {
  ...
  version=$(awk -F '"' '/__version__/ {print $2}' compute/__init__.py | tr -d ' ')
...
if [ -d "$DIR" ]; then
    ...
    if git rev-parse --git-dir > /dev/null 2>&1; then
        ...
        remote_version=$(git show origin/main:compute/__init__.py | awk -F '"' '/__version__/ {print $2}' | tr -d ' ')
